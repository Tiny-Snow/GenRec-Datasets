...
Our **GenRec Benchmark** provides a series of standardized datasets for **generative/sequential recommendation**. These datasets are crawled from various open sources, including Amazon, Douban, Gowalla, MovieLens, Yelp, etc. For those review datasets where text metadata is available, we also process the item title data for LLM/SID-based generative recommendation.
...
The dataset processing methods are provided in the [process_data/base_dataset.py](https://github.com/Tiny-Snow/GenRec-Datasets/tree/main/process_data/base_dataset.py). Basically, we will process the dataset into the following steps:
...
- **Sample users**: implemented in the `DatasetProcessor._sample_users()` method. If the dataset is too large (especially for the LLM/SID-based generative recommendation), we may sample the users to reduce the dataset size. Note that the final dataset usually has smaller user size than the number specified in this step, since some users may be filtered out in the later steps (e.g., $K$-core filtering).
...
**Customization.** You can modify and run [run_process_data.sh](https://github.com/Tiny-Snow/GenRec-Datasets/tree/main/run_process_data.sh) to customize your dataset processing.
...
| ------- | ----: | ----: | -----------: | ------: | ----------------------: | ----------------: |
| [Amazon-2014](#amazon) | 21,357 | 27,905 | 1,834,639 | 0.00308 | 85.90 | N/A |
...
| [Amazon-2018](#amazon) | 377,157 | 245,316 | 22,310,270 | 0.00024 | 59.15 | N/A |
